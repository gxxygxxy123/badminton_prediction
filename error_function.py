...
def space_time_err(gt_curve, pd_curve, bidirectional=False):
    ...
    for P in pd_curve:
        ...
        if P[-1] >= gt_curve[-1,-1]:
            ...
            A = gt_curve[low-1]
            B = gt_curve[low]
            assert P[-1] >= A[-1] and P[-1] <= B[-1], f" {B[-1]} >= {P[-1]} >= {A[-1]}"
...
def time_after_err(gt_curve, pd_curve, gt_fps, pd_fps, time):
    # ground truth & predict curve shape: (?, 3) or (?, 4), must include timestamp
    assert (gt_curve.ndim == 2 and pd_curve.ndim == 2 and
            gt_curve.shape[1] == pd_curve.shape[1] and
            (gt_curve.shape[1] == 3 or gt_curve.shape[1] == 4)), "Wrong"
    # output: after TIME, the point error
    if math.ceil(time*gt_fps) < gt_curve.shape[0] and math.ceil(time*pd_fps) < pd_curve.shape[0]:
        gt_position = gt_curve[math.floor(time*gt_fps)] * (1-math.modf(time*gt_fps)[0]) + \
                    gt_curve[math.ceil(time*gt_fps)]  * (math.modf(time*gt_fps)[0])     \
                    / 1
        pd_position = pd_curve[math.floor(time*pd_fps)] * (1-math.modf(time*pd_fps)[0]) + \
                    pd_curve[math.ceil(time*pd_fps)]  * (math.modf(time*pd_fps)[0])     \
                    / 1
        return np.linalg.norm(gt_position[:-1]-pd_position[:-1])
    else:
        return np.nan
"""
